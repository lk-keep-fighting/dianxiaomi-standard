...
def collect_env_values(args: argparse.Namespace) -> Dict[str, str]:
    ...
    if missing:
        raise RuntimeError(
            "缺少必要的 Supabase 配置: " + ", ".join(missing) +
            "。可以通过参数或 .env 文件提供，例如: --supabase-url ... --supabase-api-key ..."
        )
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--supabase-url", help="Supabase 项目的 RESTful 访问地址")
    parser.add_argument("--supabase-api-key", help="Supabase 服务密钥")
    parser.add_argument(
        "--env-file",
        action="append",
        default=[],
        help="附加的 .env 文件路径（可多次指定，后者覆盖前者）",
    )
    parser.add_argument(
        "--set",
        action="append",
    ...
    parser.add_argument(
        "--keep-embedded",
        action="store_true",
        help="打包完成后保留写入的 _embedded_env.py（默认会自动还原）",
    )

    args = parser.parse_args(argv)

    if platform.system() != "Windows":
        print("⚠️ 当前系统并非 Windows。生成的可执行文件仍然以 Windows 为目标，但建议在 Windows 环境中执行打包。")

    env_values = collect_env_values(args)
    print("🔐 嵌入环境变量:", ", ".join(sorted(env_values.keys())))
    ...
    try:
        ...
        if args.keep_embedded:
            print("ℹ️ 按照参数要求，保留生成的 _embedded_env.py 文件。")
        else:
